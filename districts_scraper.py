...
fips_state_codes = json.load(open('./dictionaries/fips_state_codes.json'))
...
for num in range(1, 57):
    ...
    for endpoint in endpoints.keys():
        ...
        for district in results:
            district_num = int(district[2])
            districts[district_num] = {}
            districts[district_num]['number'] = district_num
            districts[district_num][endpoints[endpoint]] = district[0]
    ...
    for district_key in districts.keys():
        ...
        dist = District(
            alpha_num = abbrev + '-' + district_key,
            state = num,
            number = district_key,
            representative_id = response['id'],
            population = district['population'],
            median_age = district['median_age'],
            median_age_men = district['median_age_men'],
        ...
        state_result = Distritc.query.filter(
